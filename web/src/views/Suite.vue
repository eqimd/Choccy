<template>
  ...
  >
    ...
    <el-table-column prop="Extra.suite_description" label="Description" sortable="custom"/>
    <el-table-column prop="ModTime" label="Modification time"
                     sortable="custom"
  ...
  <el-dialog v-model="qlsFile.visible" :title="qlsFile.path" width="95%">
    ...
    >
      <el-descriptions-item label="Number of queries">{{ qlsFile.queries.length }}</el-descriptions-item>
    </el-descriptions>
...
const createData = () => {
  ElMessageBox.prompt('Please enter the file name', '', {
  ...
  ElMessageBox.prompt('Please enter the file name', '', {
    ...
    if(value && value.trim()!==""){
      renameSuite(oldName,value).then(response => {
        console.log(1111)
        fetchData();
        ElMessage.success("The rename was successful")
      })
    }
  })
